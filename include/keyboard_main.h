...
{
    const char *key1;          // 第一个键名
    const char *key2;          // 第二个键名
    const char *key3;          // 第三个键名
    const char *key4;          // 第四个键名
...
///////////////////////////////////////////////////////
    const char*three_key_name_ctrl_alt[53];  // 三快捷键
    ...
    const char*tow_key_name_ctrl[53];  // 双快捷键
...
void MouseDown(const char *button);  // 鼠标按下键位

void MouseUp(const char *button);  // 鼠标释放
...
void KeyDown(const char *key);  // 键盘按下

void KeyUp(const char *key);  // 键盘释放

void PressHotKey(const char *key1, const char *key2, const char *key3);  // 按下快捷键
...
void AddHotKey(const char *key1, const char *key2, const char *key3, const char *key4, void (*target_void_func)());  // 添加快捷键的函数不输入的传NULL
...
/*************** 单个按键监听判断函数 (适合游戏) ****************/
bool KeyUpListen( const char *key);  // 监听键盘抬起
bool KeyDownListen(const char *key); // 监听键盘按下
