...
# keyboard库的功能与函数介绍:
## 00:
### 仓库中的源文件main.c为配置好的示例文件，cmakelists已经配置完成，可通过修改链接源文件将其连接到自己的项目中使用。  因为作者蔡，没有实粒所以导入库需要在当前根目录的include下引入头文件:include "include/keyboard_main.h"，作者会继续奴隶，相信学满一坤年以后能够将项目越做越大，做大做强，入狱辉煌！！
...
## 7.输入函数:
### WriteStr(const char str_print[], int str_size); // 输入字符窜函数但仅可用英文。 作为一个键鼠操作的库不能进行输入怎么行呢（还是那句话，由于作者蔡，写c语言不足六个月本函数只支持输入英文，与英文的标点，如果有需求请加QQ留意，我一定尽力去找到输入中文的方法）

## 8.复制到剪切板函数:
### CopyStr(const char *str);  // 复制字符窜到剪切板,可用直接ctrl+v复制

## 9.取出剪切板数据(目前暂没有将其作为返回值返回给函数获取到的字符窜)
### PasteStr(); // 获取剪切板数据(没用上)
